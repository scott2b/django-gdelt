...
themes_p = re.compile(r'^<CATEGORY NAME="([^"]+)" TYPE="THEME">\s*<TERMS>([^<]+)</TERMS>', re.M|re.S)
...
with open('./resources/SET_EVENTPATTERNS.xml') as f:
    ...
    for theme, terms in themes_p.findall(s):
...
def get_theme(text):
    ...
    for theme, terms in themes.items():
        scores[theme] = sum([val+offset for term, val in terms if term in text])
    print(scores)
